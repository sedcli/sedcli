...
static int opal_level0_disc_pt(struct sed_device *device)
{
	...
	ret = sed_get_discv(&device->discv.sed_lvl0_discv, dev->resp_buf);
	if (ret)
		return ret;
	dev->comid = device->discv.sed_lvl0_discv.com_id;
		...
		const char *usr, enum SED_ACCESS_TYPE lock_type, uint8_t lr)
{
	...
	if (lk_type > SED_NO_ACCESS || key == NULL || key->len == 0 || usr == NULL) {
...

{
	...
	ret = opal_start_auth_session(dev->fd, opal_dev, sum, lr, who, key);
	if (ret)
		goto end_sessn;
